...
module axi_lite_regs #(
  ...
  always_comb begin
    ...
    case (s_axil_araddr)
      ...
      STATUS: begin
        read_data_mux = {{(REG_DATA_WIDTH-3){1'b0}}, error_q, done_q, busy_i};
      end
      SRC_ADDR_LO: begin
      ...
      IRQ_ENABLE: begin
        read_data_mux = {{(REG_DATA_WIDTH-2){1'b0}}, irq_enable_q};
      end
      IRQ_STATUS: begin
        read_data_mux = {{(REG_DATA_WIDTH-2){1'b0}}, irq_status_q};
      end
      VERSION: begin
  ...
  always_comb begin
    ...
    done_d        = done_q;
    error_d       = error_q;
    irq_enable_wdata = apply_wstrb({{(REG_DATA_WIDTH-2){1'b0}}, irq_enable_q},
                                   wdata_q,
                                   wstrb_q);
    ...
    if (write_fire) begin
      case (awaddr_q)
        ...
        IRQ_ENABLE: begin
          irq_enable_d = irq_enable_wdata[1:0];
        end
        IRQ_STATUS: begin
